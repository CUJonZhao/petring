...
用户明确了目标状态：**盒子绑在狗的胸背上，出门遛狗，回来登录网站就有数据和分析，中间不碰任何按钮**。由此做了两处改动并在实机上验证。

### 记录触发方式

住处 Wi-Fi 全覆盖，走到楼下也还在信号范围内，所以「离开 Wi-Fi」这一个触发不够。新增：在家范围内**持续活动约 30 秒**也开始记录，**静止 5 分钟**后自动结束。离开 Wi-Fi 的记录不受静止规则影响（狗中途休息不会被切成两段），手动开始的记录仍完全由用户控制。记录头的保留字节里保存触发原因（`w`/`m`/`u`）。

### 闭环实测（2026-09-11 23:15，室内，全程未离开家里 Wi-Fi）

| 环节 | 结果 |
|---|---|
| 触发 | 记录头触发字节 `m`，运动触发 |
| 开始／结束 | 23:15:43 开始；静止 5 分钟后自动结束，状态 `complete` |
| 采样 | 6,724 条，5.7 分钟，**19.53 Hz**，未覆盖 2.0 秒 |
| 上传 | 23:21:49 自动完成，网站收录（共 19 段） |
...
动作序列：触发用的走动 → 静止 2 分钟 → 正常走约 1.5 分钟 → 快走约 1 分钟 → 静止至自动结束。板子**端在身前、手臂不摆动**。

| 标签 | 窗口数 | MAD p10 | 中位 | p90 |
|---|---|---|---|---|
| 静止 | 83 | 0.9 mg | 1.2 mg | 4.2 mg |
| 走路 | 66 | 102.9 mg | 151.2 mg | 195.5 mg |

两类相差约 25 倍，**149 个有标签窗口全部分类正确**。阈值定为 50 mg，离线脚本与 `site/lib/gait.ts` 同步更新。
**跑步阈值仍未标定**（没有跑的数据），600 mg 是猜测值，代码与档案中均已注明。
标定档案：`experiments/results/gait_calibration_2026-09-11.json`。

### 频率特征不可用于手持场景

| 分段 | FFT 主峰 | 自相关 | 振幅 |
|---|---|---|---|
| 正常走 | 3.38 Hz | 2.86 Hz | 105–163 mg |
| 快走 | 1.88 Hz | 2.00 Hz | 165–204 mg |

两种估计方法一致，所以不是算法挑错谐波。快走的 2.0 Hz 即 120 步/分，是真实步频；正常走的约 3 Hz 太快，不可能是步频——端在身前时手臂构成弹簧，其 2–5 Hz 晃动在躯干起伏较小时盖过了步频。**该频率反映的是携带方式，不是步态**，因此分类改为只用振幅（振幅排序正确：快走 > 正常走 > 静止），频率降级为诊断量。

胸背固定是刚性耦合到躯干的，预期主频就是步频；**第一次佩戴记录回来后要专门复核这一点**，若频率变可靠，跑步判定可同时使用振幅与步频。

另外修了一个假阳性：放下板子瞬间的单个 610 mg 窗口被判为「跑」，现在首尾窗口跟随相邻窗口。
...
1. **在 Codex 中部署 `site/`**（含标定后的阈值与分析面板）。本机 `npx`／node 不在登录 shell 的 PATH 中，`bash experiments/cloud_sync_mac.sh sitecheck` 会自动寻找 node；找不到时直接由 Codex 构建部署。部署后那段记录应显示「走路 2 分 42 秒 · 跑动 0 秒 · 活动指数 80 mg」，与离线脚本一致。
2. **把盒子绑在 Delta 的胸背上遛一次**。回来后重点看三件事：刚性固定下频率是否变成可用的步频；狗的振幅区间落在哪里（据此重新标定走／跑）；整趟的电池消耗。分析命令：
   `python3 experiments/gait_analysis.py <记录>.bin --plot ... --windows-csv ...`，带标签时再加 `--labels ... --suggest`。
3. 一小时电池记录；评估外出时是否关闭热点以省电。
...
| 活动分析（离线） | [步态与活动量分析](../experiments/gait_analysis.py)、[标定档案](../experiments/results/gait_calibration_2026-09-11.json) |
| 网站端分析 | `site/lib/gait.ts`、`site/app/dashboard.tsx`（独立仓库，未推送到 GitHub） |
